...
import com.za.toptitup.loginlibrary.sdk.Command;
...
public class PrintDemo extends Activity implements OnClickListener{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		btn_printA = findViewById(R.id.btn_bmp);
		...
		btn_BMP = findViewById(R.id.btn_printpicture);
	...
	/*	switch (v.getId()) {
		...
		if (v.getId() == R.id.btn_conn) {
			...
			if (hexBox.isChecked()) {
				String str = txt_content.getText().toString().trim(); // Remove leading and trailing spaces
				if (str.length() > 0) {
					str = Other.RemoveChar(str, ' ').toString();
					if (str.length() <= 0)
						return;
			...
			if ((lang.compareTo("ch")) == 0) {
				new AlertDialog.Builder(PrintDemo.this)
						.setTitle(getText(R.string.chosecommand))
						.setItems(items, new DialogInterface.OnClickListener() {
							public void onClick(DialogInterface dialog, int which) {
								SendDataByte(byteCommands[which]);
								try {
